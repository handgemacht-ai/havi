...
func TestInstall_OnMissingFile_CreatesPrettyFormatted(t *testing.T) {
	...
	cmd := gjson.GetBytes(got, "mcpServers.havi.command").String()
	if cmd != "havi" {
		t.Errorf("mcpServers.havi.command = %q, want %q", cmd, "havi")
	}
	args := gjson.GetBytes(got, "mcpServers.havi.args").Array()
	if len(args) != 1 || args[0].String() != "mcp-bridge" {
		t.Errorf("mcpServers.havi.args = %v, want [\"mcp-bridge\"]", args)
	}
...
func TestInstall_OnEmptyJSONFile_PrettyFormatted(t *testing.T) {
	dir := t.TempDir()
	...
	if !bytes.Contains(got, []byte("\n  \"mcpServers\"")) {
		t.Errorf("expected pretty-formatted output; got:\n%s", got)
	}
}
...
`)
	...
	got, _ := os.ReadFile(path)
	if !gjson.GetBytes(got, "mcpServers.other.command").Exists() {
		t.Errorf("unrelated mcpServers.other entry was lost:\n%s", got)
	}
	if gjson.GetBytes(got, "mcpServers.other.command").String() != "other-cli" {
		t.Errorf("unrelated entry value changed; got:\n%s", got)
	}
	if !gjson.GetBytes(got, "mcpServers.havi").Exists() {
...
func TestInstall_OnFileWithOtherTopLevelKeys_PreservesThem(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "mcp.json")
	seed := []byte(`{"settings":{"theme":"dark"},"mcpServers":{}}`)
	if err := os.WriteFile(path, seed, 0o644); err != nil {
		t.Fatalf("seed: %v", err)
	}
	...
	got, _ := os.ReadFile(path)
	if gjson.GetBytes(got, "settings.theme").String() != "dark" {
...
func TestInstall_ReplacesDriftedHaviEntry(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "mcp.json")
	seed := []byte(`{"mcpServers":{"havi":{"command":"wrong-binary","args":["wrong"]}}}`)
	if err := os.WriteFile(path, seed, 0o644); err != nil {
		t.Fatalf("seed: %v", err)
	}
	...
	got, _ := os.ReadFile(path)
	if gjson.GetBytes(got, "mcpServers.havi.command").String() != "havi" {
		t.Errorf("drifted command not replaced; got:\n%s", got)
	}
	if gjson.GetBytes(got, "mcpServers.havi.args.0").String() != "mcp-bridge" {
...
func TestUninstall_OnFileWithoutHavi_IsNoOp(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "mcp.json")
	seed := []byte(`{"mcpServers":{"other":{"command":"other-cli"}}}`)
	if err := os.WriteFile(path, seed, 0o644); err != nil {
		t.Fatalf("seed: %v", err)
	}
	mtime1, _ := mtime(path)
...
func TestConfigPath_HonorsEnvOverride(t *testing.T) {
	t.Setenv("HAVI_CURSOR_CONFIG", "/tmp/custom/mcp.json")
	got, err := ConfigPath()
	if err != nil {
		t.Fatalf("ConfigPath: %v", err)
	}
	if got != "/tmp/custom/mcp.json" {
		t.Errorf("ConfigPath = %q, want %q", got, "/tmp/custom/mcp.json")
	}
}
...
func TestInstall_OnMalformedJSON_ReturnsError(t *testing.T) {
	...
	if err != nil && !strings.Contains(err.Error(), "mcpServers.havi") {
		// Loose check: sjson surfaces the path in its error.
